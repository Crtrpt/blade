  ...
  try {
      ...
      if (keys.length) {
        ...
        /* COMPONENT */
          ...
          if (extraAttrs.length) {
            warn$1$1(`Extraneous non-props attributes (` + `${extraAttrs.join(', ')}) ` + `were passed to component but could not be automatically inherited ` + `because component renders fragment or text root nodes.`);
          }

          if (eventAttrs.length) {
            warn$1$1(`Extraneous non-emits event listeners (` + `${eventAttrs.join(', ')}) ` + `were passed to component but could not be automatically inherited ` + `because component renders fragment or text root nodes. ` + `If the listener is intended to be a component custom event listener only, ` + `declare it using the "emits" option.`);
          }
        }
      }
    }
    ...
    if (vnode.dirs) {
      if (true && !isElementRoot(root)) {
        warn$1$1(`Runtime directive used on component with non-element root node. ` + `The directives will not function as intended.`);
      }
    ...
    if (vnode.transition) {
      if (true && !isElementRoot(root)) {
        warn$1$1(`Component inside <Transition> renders non-element root node ` + `that cannot be animated.`);
      }
...
function injectHook(type, hook, target = currentInstance, prepend = false) {
  if (target) {
    ...
    warn$1$1(`${apiName} is called when there is no active component instance to be ` + `associated with. ` + `Lifecycle injection APIs can only be used during execution of setup().` + (` If you are using async setup(), make sure to register lifecycle ` + `hooks before the first await statement.`));
  }
}
...
const normalizeSlot$1 = (key, rawSlot, ctx) => {
  const normalized = withCtx((...args) => {
    if (currentInstance) {
      warn$1$1(`Slot "${key}" invoked outside of the render function: ` + `this will not track dependencies used in the slot. ` + `Invoke the slot function inside the render function instead.`);
    }
...
const normalizeVNodeSlots = (instance, children) => {
  if (!isKeepAlive(instance.vnode) && !false) {
    warn$1$1(`Non-function value encountered for default slot. ` + `Prefer function slots for better performance.`);
  }
      ...
      for (i = s2; i <= e2; i++) {
        ...
        if (nextChild.key != null) {
          if (keyToNewIndexMap.has(nextChild.key)) {
            warn$1$1(`Duplicate keys found during update:`, JSON.stringify(nextChild.key), `Make sure keys are unique.`);
          }
...
function setRef(rawRef, oldRawRef, parentSuspense, vnode, isUnmount = false) {
  ...
  if (isString(ref)) {
    ...
    warn$1$1('Invalid template ref type:', value, `(${typeof value})`);
  }
}
...
function resolveAsset(type, name, warnMissing = true, maybeSelfReference = false) {
  ...
  if (instance) {
    ...
    if (warnMissing && !res) {
      warn$1$1(`Failed to resolve ${type.slice(0, -1)}: ${name}`);
    }
...
function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, isBlockNode = false) {
  ...
  && isProxy(type)) {
    type = toRaw(type);
    warn$1$1(`Vue received a Component which was made a reactive object. This can ` + `lead to unnecessary performance overhead, and should be avoided by ` + `marking the component with \`markRaw\` or using \`shallowRef\` ` + `instead of \`ref\`.`, `\nComponent that was made reactive: `, type);
  }
...
function finishComponentSetup(instance, isSSR, skipOptions) {
  ...
  if (!Component.render && instance.render === NOOP && !isSSR) {
    ...
    if (Component.template) {
      warn$1$1(`Component provided template option but ` + `runtime compilation is not supported in this build of Vue.` + ` Use "vue.esm-browser.js" instead.`
      /* should not happen */
      );
    } else {
...
prevChildren, parentComponent, parentSuspense, unmountChildren) {
  ...
  try {
    ...
    {
      warn$1$1(`Failed setting prop "${key}" on <${el.tagName.toLowerCase()}>: ` + `value ${value} is invalid.`, e);
    }
  }
...
function injectCompilerOptionsCheck(app) {
  {
    ...
    Object.defineProperty(app.config, 'isCustomElement', {
      ...
      set() {
        warn$1$1(`The \`isCustomElement\` config option is deprecated. Use ` + `\`compilerOptions.isCustomElement\` instead.`);
      }
...
function normalizeContainer(container) {
  if (isString(container)) {
    ...
    if (!res) {
      warn$1$1(`Failed to mount app: mount target selector "${container}" returned null.`);
    }
  ...
  if (window.ShadowRoot && container instanceof window.ShadowRoot && container.mode === 'closed') {
    warn$1$1(`mounting on a ShadowRoot with \`{mode: "closed"}\` may lead to unpredictable bugs`);
  }
...
script$1.__file = "src/layout/Master.vue";
...
var route = [
    ...
    {
        path: "/home",
        component: () => import('./Home-ea5120c5.js'),
    },
];
...
export { createBaseVNode as a, createElementBlock as c, openBlock as o };
//# sourceMappingURL=main-8947d36b.js.map
