...
var Vnode$6 = vnode; // Call via `hyperscriptVnode.apply(startOffset, arguments)`
//
...
var render$2 = function ($window) {
  ...
  function createElement(parent, vnode, hooks, ns, nextSibling) {
    ...
    if (!maybeSetContentEditable(vnode)) {
      if (vnode.text != null) {
        if (vnode.text !== "") element.textContent = vnode.text;else vnode.children = [Vnode$2("#", undefined, undefined, vnode.text, undefined, undefined)];
      }
...
var mountRedraw$4 = function (render, schedule, console) {
  ...
  function sync() {
    ...
    for (var i = 0; i < subscriptions.length; i += 2) {
      try {
        render(subscriptions[i], Vnode$1(subscriptions[i + 1]), redraw);
      } catch (e) {
        console.error(e);
      }
    }
...
var assign$1 = assign$2; // Returns `path` from `template` + `params`
...
var parseQueryString = parse$1; // Returns `{path, params}` from `url`
...
var parsePathname$1 = parse; // Compiles a template into a function that takes a resolved path (without query
...
var router = function ($window, mountRedraw) {
  ...
  route.Link = {
    view: function (vnode) {
      ...
      var child = m$1(vnode.attrs.selector || "a", attrs, vnode.children); // Let's provide a *right* way to disable a route link, rather than
...
var route$1 = router(window, mountRedraw$1);
...
class Index {
    view(vnode) {
        return (mithril(Master, null,
            mithril("a", { href: "#!/login" },
                "\u9996\u9875",
                mithril("button", { className: "bg-gray-400" }, "button"))));
    }
}
...
class Login {
    view(vnode) {
        console.log(vnode);
        return (mithril(Black, null,
            mithril("div", { class: "flex flex-col " },
                mithril("div", { className: "text-2xl" }, "Blade"),
                mithril("div", { className: "text-1xl mt-5" }, "\u7528\u6237\u767B\u5F55"),
                mithril("div", { className: "mt-10 " },
                    mithril("form", { className: "" },
                        mithril("span", { class: "text-gray-700" }, "\u7528\u6237\u540D:"),
                        mithril("input", { type: "email", class: "form-input mx-5 px-4 py-3 rounded-full", placeholder: "\u8BF7\u8F93\u5165\u7528\u6237\u540D" }),
                        mithril("span", { class: "text-gray-700" }, "\u5BC6\u7801:"),
                        mithril("input", { type: "password", class: "form-input mx-5 px-4 py-3 rounded-full", placeholder: "\u8BF7\u8F93\u5165\u5BC6\u7801" }),
                        mithril("input", { type: "submit", class: "form-input mx-5 px-4 py-3 rounded-full bg-primary text-white", value: "\u767B\u5F55" }))))));
    }
}
...
class bladeui {
    constructor(config) {
        var root = config.el;
        mithril.route(root, "/", route);
    }
}

export { bladeui as default };
